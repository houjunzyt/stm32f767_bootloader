...
SD_Error SD_Init(void)
{
	...
	RCC->APB2ENR|=1<<11;		//SDMMC1时钟使能
	RCC->APB2RSTR|=1<<11;		//SDMMC1复位
	...
	GPIO_Set(GPIOC,0X1F<<8,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU);	//PC8,9,10,11,12复用功能输出	
 	GPIO_Set(GPIOD,1<<2,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU);		//PD2复用功能输出	
	RCC->APB2RSTR&=~(1<<11);	//SDMMC1结束复位
 	//SDMMC外设寄存器设置为默认值 			   
...
//SDMMC时钟初始化设置
//clkdiv:时钟分频系数
//CK时钟=SDMMCCLK/[clkdiv+2];(SDMMCCLK时钟固定为48Mhz)
...
SD_Error SD_PowerON(void)
{
	...
	//配置CLKCR寄存器  
	tempreg|=0<<9;				//非省电模式
	tempreg|=0<<10;				//关闭旁路,CK根据分频设置输出
	tempreg|=0<<11;				//1位数据宽度
	tempreg|=0<<13;				//SDMMCCLK上升沿产生SDMMCCK
	tempreg|=0<<14;				//关闭硬件流控制    
	SDMMC1->CLKCR=tempreg; 
	SDMMC_Clock_Set(SDMMC_INIT_CLK_DIV);//设置时钟频率(初始化的时候,不能超过400Khz)			 
 	SDMMC1->POWER=0X03;			//上电状态,开启卡时钟     
  	SDMMC1->CLKCR|=1<<8;		//SDMMCCK使能   
   	for(i=0;i<74;i++)
	{
...
SD_Error SD_ReadBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
	...
	{ 
		...
		while(SDMMC1->STA&(1<<21))	//FIFO里面,还存在可用数据
		{
			*tempbuff=SDMMC1->FIFO;	//循环读取数据
			tempbuff++;
		} 
	...
	}else if(DeviceMode==SD_DMA_MODE)
	{
		...
		SDMMC1->MASK|=(1<<1)|(1<<3)|(1<<8)|(1<<5);	//配置需要的中断 
		SDMMC1->DCTRL|=1<<3;						//SDMMC DMA使能 
...
SD_Error SD_WriteBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
	...
	}else if(DeviceMode==SD_DMA_MODE)
	{
		...
		TransferEnd=0;				//传输结束标置位，在中断服务置1
		SDMMC1->MASK|=(1<<1)|(1<<3)|(1<<8)|(1<<4);		//配置产生数据接收完成中断
		SD_DMA_Config((u32*)buf,nblks*blksize,1);		//SDMMC DMA配置
 	 	SDMMC1->DCTRL|=1<<3;							//SDMMC DMA使能.  
 		while(((DMA2->LISR&(1<<27))==RESET)&&timeout)timeout--;//等待传输完成 
		if(timeout==0)
		{
...
{											
 	SD_ProcessIRQSrc();//处理所有SDMMC1相关中断
}	 																    
//SDMMC中断处理函数
//处理SDMMC传输过程中的各种中断事务
//返回值:错误代码
SD_Error SD_ProcessIRQSrc(void)
{
	...
	{	 
		if (StopCondition==1)
		{
 		...
 		SDMMC1->ICR|=1<<8;//清除完成中断标记
		SDMMC1->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5));//关闭相关中断
 		TransferEnd = 1;
		return(TransferError);
	}
 	if(SDMMC1->STA&(1<<1))//数据CRC错误
	{
		SDMMC1->ICR|=1<<1;//清除中断标记
		SDMMC1->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5));//关闭相关中断
 	...
 	if(SDMMC1->STA&(1<<3))//数据超时错误
	{
		SDMMC1->ICR|=1<<3;//清除中断标记
		SDMMC1->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5));//关闭相关中断
	    TransferError = SD_DATA_TIMEOUT;
	    return(SD_DATA_TIMEOUT);
	}
  	if(SDMMC1->STA&(1<<5))//FIFO上溢错误
	{
		SDMMC1->ICR|=1<<5;//清除中断标记
		SDMMC1->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5));//关闭相关中断
	    TransferError = SD_RX_OVERRUN;
	    return(SD_RX_OVERRUN);
	}
   	if(SDMMC1->STA&(1<<4))//FIFO下溢错误
	{
		SDMMC1->ICR|=1<<4;//清除中断标记
		SDMMC1->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5));//关闭相关中断
...
{ 
	...
	{ 
		if(SDMMC1->STA&(1<<21))//接收FIFO数据可用
		{
			*(tempscr+index)=SDMMC1->FIFO;	//读取FIFO内容
			index++;
			if(index>=2)break;
		}
	}
...
//mbuf:存储器地址
//bufsize:传输数据量
//dir:方向;1,存储器-->SDMMC(写数据);0,SDMMC1-->存储器(读数据);
...
{		 
	...
	DMA2_Stream3->PAR=(u32)&SDMMC1->FIFO;	//DMA2 外设地址
