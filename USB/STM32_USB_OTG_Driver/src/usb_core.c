...
#if defined (STM32F446xx) || defined (STM32F469_479xx) || defined (STM32F767xx)	//增加对STM32F767xx的判断
  //必须注释掉这个(对usbcfg.srpcap的修改),否则无法正常使用!!
  //usbcfg.d32 = USB_OTG_READ_REG32(&pdev->regs.GREGS->GUSBCFG);
  //usbcfg.b.srpcap = 1;
  //必须新增对GOTGCTL寄存器bit6,bit7的设置,否则USB工作不正常
  tempreg=USB_OTG_READ_REG32(&pdev->regs.GREGS->GOTGCTL);	//读取GOTGCTL寄存器
  tempreg|=1<<6;	//设置BVALOEN=1
  tempreg|=1<<7;	//设置BVALOVAL=1
  USB_OTG_WRITE_REG32(&pdev->regs.GREGS->GOTGCTL,tempreg);	//修改GOTGCTL寄存器的值
